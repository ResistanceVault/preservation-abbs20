;/*
sc5 -j73 -q5e -d4 UserEditor
copy UserEditor.c -J:UserEditor.c
slink LIB:c.o+"UserEditor.o" to UserEditor LIB LIB:sc.lib LIB:JEO.lib ADDSYM
quit
*/

/***************************************************************************
*									UserEditor 2.00 (1/05-94)
*
*	Edits userlists
*
*
...
#include "dabbs:JEO/UserEditorGui.c"
...
char *vers = "\0$VER: UserEditor 1.00 (29.05.97)\n\r";	/* day,month,year */
...
int main (int argc, char **argv)
{
	...
	if (Setup())
	{
		...
		else
		{
			...
			curuser = AllocMem (config->UserrecordSize, MEMF_CLEAR);
			curuserback = AllocMem (config->UserrecordSize, MEMF_CLEAR);
			memcpy (curuserback, curuser, config->UserrecordSize);
...
void updateconflistview(void)
{
	...
	for (n = 0; n < config->Maxconferences; n++)
	{
		ptr = bits;
		ptr2 = "RWUDFISZ";
		acc = curuser->firstuserconf[confarray[n].n_ConfOrder-1].uc_Access;
		...
		bits[8] = '\0';
		sprintf (nodes[n].text,"%s-%s",bits, confarray[confarray[n].n_ConfOrder-1].n_ConfName);
//			config->ConfNames[config->ConfOrder[n]-1]);
	}
...
int setupconflistview (void)
{
	...
	for (n = 0; n < numconfs; n++)
	{
		...
		sprintf (nodes[n].text,"        -%s", confarray[confarray[n].n_ConfOrder-1].n_ConfName);
//			config->ConfNames[config->ConfOrder[n]-1]);
...
int setuplistview(void)
{
	...
	if (in = Open (UserFName, MODE_OLDFILE))
	{
		for (n = 0; n < num; n++)
		{
			...
			Read (in, curuser, config->UserrecordSize);
			strcpy (nodes[n].name, curuser->Name);
